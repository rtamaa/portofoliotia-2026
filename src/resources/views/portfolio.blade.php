...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Header */
        header { position: fixed; top: 0; left: 0; right: 0; display: flex; justify-content: space-between; align-items: center; padding: 1.5rem 2rem; z-index: 100; background: rgba(10,10,10,0.95); backdrop-filter: blur(10px); }
        ...
        .desktop-nav a { text-decoration: none; color: #fff; font-weight: 400; transition: color 0.3s; }
        .desktop-nav a:hover { color: #ff3366; }
        ...
        /* Split Container */
        .split-container { display: flex; min-height: 100vh; padding-top: 80px; }
        ...
        /* Controls */
        .project-controls { position: fixed; bottom: 2rem; right: 2rem; display: flex; gap: 1rem; z-index: 100; }
        .progress-indicator { display: flex; gap: 0.5rem; align-items: center; background: rgba(255,255,255,0.1); padding: 0.5rem 1rem; border-radius: 30px; backdrop-filter: blur(10px); }
        .progress-dot { width: 8px; height: 8px; border-radius: 50%; background: rgba(255,255,255,0.3); cursor: pointer; transition: 0.3s; }
        .progress-dot.active { width: 24px; background: #ff3366; border-radius: 4px; }
        .navigation { display: flex; gap: 0.5rem; }
        .nav-arrow { width: 40px; height: 40px; background: rgba(255,255,255,0.1); border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; backdrop-filter: blur(10px); transition: 0.3s; }
        .nav-arrow:hover { background: #ff3366; }
        .arrow { width: 12px; height: 12px; border-top: 2px solid #fff; border-right: 2px solid #fff; }
...
<body>
    ...
    <div class="mobile-nav">
        ...
        <div class="mobile-nav-footer">
            ...
            <a href="/cdn-cgi/l/email-protection#7a121f1616153a1f021b170a161f54191517"><span class="__cf_email__">[email&#160;protected]</span></a>
        </div>
    </div>
    ...
    <div class="split-container" id="work">
        ...
        <div class="right-panel">
            ...
            <div class="project-details {{ $index === 0 ? 'active' : '' }}" data-project="{{ $index }}">
                ...
                <a href="{{ route('project.show', $project->slug) }}" class="view-project-btn"> Lihat Detail Proyek →</a>
            </div>
            @endforeach
        </div>
    ...
    <script>
        // Project slider
        const projects = document.querySelectorAll('.image-container, .project-details');
        ...
        function showProject(index) {
            ...
            document.querySelectorAll(`.image-container[data-project="${index}"]`).forEach(el => el.classList.add('active'));
            document.querySelectorAll(`.project-details[data-project="${index}"]`).forEach(el => el.classList.add('active'));
            dots.forEach((dot, i) => {
        ...
        function nextProject() { let next = (currentIndex + 1) % totalProjects; showProject(next); }
        function prevProject() { let prev = (currentIndex - 1 + totalProjects) % totalProjects; showProject(prev); }
